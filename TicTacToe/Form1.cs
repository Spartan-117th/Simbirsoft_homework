...
namespace TicTacToe
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void BTN_Click(object sender, EventArgs e)      //Clicking on a cell
        {
            if (!is_row && !is_col && !is_diag && !is_full)     //If game is not over
            {
                ...
                //statusRTB.Text += "Ход " + turn_num.ToString() + ". " + s + " сделал ход по координатам " + b.Row.ToString() + ":" + b.Col.ToString() + ".\n";
                //statusRTB.SelectionStart = statusRTB.Text.Length;
                //statusRTB.ScrollToCaret();                                    Replaced with gameStatusBox with IStatus
                ...
                {                 
                    ...
                    //statusRTB.Text += "Игра окончена!\n";
                    //statusRTB.SelectionStart = statusRTB.Text.Length;             //Replaced with gameStatusBox with IStatus
    ...
    {
        int Row { get; set; }                       //Row position
        int Col { get; set; }                       //Column position
        string PlaceMark(string s);                 //Required mark
    }


    public class Cell : Button, IMark               //Updating the Button with IMark => Cell
    {
        private int row;
        public int Row
        {
    ...
    public interface IStatus
    {
        ...
        void WriteStatus_Start(string mark);                                            //Start status
    ...
    public class StatusBox : RichTextBox, IStatus               //Updating the RichTextBox with IStatus => StatusBox
    {
        ...
        public void WriteStatus_Start(string mark)
        {
